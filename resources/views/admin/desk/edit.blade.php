...
@section('content')
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active" aria-current="page">{{ $desk->name }}</li>
        </ol>
    </nav>
        ...
        <div class="col-lg-6">
            <div class="card shadow mb-4">
                <div class="card-header py-3">
                    <h5 class="m-0 font-weight-bold text-primary">Edit Desk</h5>
                </div>
                <div class="card-body">
                    @if (count($errors) > 0)
                    ...
                    @endif
                    <form action="{{ route('floor.sector.desk.update', [$floor->id, $sector->id, $desk->id]) }}"
                        method="POST">
                        @csrf
                        @method('PUT')
